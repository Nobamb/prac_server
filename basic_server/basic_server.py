...
# 최상위 폴더를 찾도록 하기
# 현재 파일을 기준으로 2번의 부모폴더 찾기
# basic.server.py(절대 경로) => basic.server(폴더) => prac_server(폴더)

# find_path(현재 파일의 위치를 절대 경로로)
find_path = os.path.abspath(__file__)
# parent_path(find_path의 부모 폴더)
parent_path = os.path.dirname(find_path)
# grand_parent_path(parent_path의 부모 폴더)
grand_parent_path = os.path.dirname(parent_path)

# 시스템 경로에 grand_parent_path추가
